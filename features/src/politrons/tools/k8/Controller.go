...
func NewNamespaceController(kclient *kubernetes.Clientset) *NamespaceController {
	...
	namespaceInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: namespaceWatcher.createRoleBindingByNamespace,
	})
...
func (controller *NamespaceController) deleteRoleBindingInNameSpace(namespaceName string, roleBinding *rbacV1.RoleBinding) {
	...
	if err != nil {
		log.Println(fmt.Sprintf("Failed to delete Role Binding: %s", err.Error()))
	} else {
		log.Println(fmt.Sprintf("Delete AD RoleBinding for Namespace: %s", roleBinding.Name))
	}
}
